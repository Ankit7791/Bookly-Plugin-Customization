...
?>
  <?php 
                                      ...
                                      $taxrate = 11;    
                                      $csubtotal = $cart_info->getSubtotal();
                                      $taxsubtotal = $csubtotal / $taxrate;
                                      $actualsubtotal = $csubtotal -  $taxsubtotal;
                                      
                                ?> 
<?php $table['subtotal'] = $cart_info->getSubtotal(); ?>
    ...
    <div class="bookly-cart bookly-box">
        <table>
            ...
            <?php foreach ( $table['rows'] as $key => $data ) : ?>
                ...
                <?php BookingProxy\Discounts::renderCartItemInfo( $userData, $key, $table['header_position'], false ) ?>
            ...
            <?php if ( isset( $table['header_position']['price'] ) || ( $table['show']['deposit'] && isset( $table['header_position']['deposit'] ) ) ) : ?>
                ...
                <?php if ( isset ( $table['header_position']['price'] ) ) : ?>
                    ...
                    <?php if ( $table['show']['tax'] ) : ?>
                        <tr>
                            <th><?php esc_html_e( 'GST', 'bookly' ) ?>:</th>
                            <td><strong class="bookly-js-total-tax"><?php echo Price::format( $taxsubtotal ) ?></strong></td>
                        </tr>
                    <?php endif ?>
                <?php endif ?>
                <?php BookingProxy\DepositPayments::renderPayNowRow( $cart_info, $table, 'mobile' ) ?>
                 ...
                 <tr> 
                    <td> <strong><?php esc_html_e( 'Remaining Amount', 'bookly' ) ?>:</strong> </td> 
                    <td class="bookly-rtext"><strong class="bookly-js-subtotal-price">
                        <?php 
                             $remainingamt = $cart_info->getTotal() - $cart_info->getPayNow();
                             echo Price::format( $remainingamt );
                        ?>
                    ...
                    <?php foreach ( $table['headers'] as $position => $column ) : ?>
                        <td <?php if ( isset ( $table['header_position']['price'] ) && $position == $table['header_position']['price'] ) echo 'class="bookly-rtext"' ?>>
                            ...
                            <?php if ( isset( $table['header_position']['price'] ) && $position == $table['header_position']['price'] ) : ?>
                                ...
                                <strong class="bookly-js-subtotal-price"><?php echo Price::format($actualsubtotal) ?></strong>
                            <?php endif ?>
                            <?php if ( $table['show']['deposit'] && $position == $table['header_position']['deposit'] ) : ?>
                ...
                <tr> 
                    <td> <strong><?php esc_html_e( 'GST', 'bookly' ) ?>:</strong> </td> 
                    <td></td>
                    <td></td>
                    <td class="bookly-rtext"><strong class="bookly-js-subtotal-price"><?php echo Price::format($taxsubtotal) ?></strong></td>
                </tr>
                
                <!--<tr> <td> <strong class="bookly-js-subtotal-deposit"><?php //echo Price::format( $cart_info->getDeposit() ) ?></strong></td> </tr>-->
                ...
                <tr>
                    <?php foreach ( $table['headers'] as $position => $column ) : ?>
                        <td <?php if ( isset( $table['header_position']['price'] ) && $position == $table['header_position']['price'] ) echo 'class="bookly-rtext"' ?>>
                            <?php if ( $position == 0 ) : ?>
                                <strong><?php esc_html_e( 'Total Amount', 'bookly' ) ?>:</strong>
                            <?php endif ?>
                            <?php if ( isset( $table['header_position']['price'] ) && $position == $table['header_position']['price'] ) : ?>
                                <strong class="bookly-js-total-price"><?php echo Price::format( $cart_info->getTotal() ) ?></strong>
                            <?php endif ?>
                            <?php if ($position != 5 && $table['show']['tax'] && $position == $table['header_position']['tax'] ) : ?>
