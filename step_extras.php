...
$amount = 0;
?>
<?php echo "<h2 style='color:#ff0000'> Selected Package : ".($chain[0]['service_title'])." Cleaning </h2> "; ?> 
<h3><strong></strong>Check Package Inculsions <a href="https://www.cleanersaustralia.com.au/cleaning-inclusions/" target="_blank" style="color:#ff0000"> Here </a> </strong></h3>

 <?php if ( in_array( 'summary', $show ) ) : ?>
        <div class="bookly-box bookly-extras-summary bookly-js-extras-summary">
            <strong style="font-size:20px">
            <?php esc_html_e( 'Total Price', 'bookly' ) ?>:<?php if ( $chain_price !== null ): ?> <?php echo Price::format( $chain_price ) ?><?php endif ?><span><?php echo $amount ? ' + ' . Price::format( $amount ) : '' ?></span>
            </strong></div>
    <?php endif ?>

<div class="bookly-box"><h3 style="color:#99c83c;font-size:16px;font-weight:bold"><?php echo $info_text ?></h3></div>
...
<div class="bookly-extra-step">
        ...
        <?php if ( ! empty( $chain_item['extras'] ) ) : ?>
            ...
            <?php foreach ( $chain_item['extras'] as $extra ) :
                ...
                <div class="bookly-extras-item bookly-js-extras-item" data-id="<?php echo $extra_id ?>" data-price="<?php echo $extra_price ?>" data-max_quantity="<?php echo $extra->getMaxQuantity() ?>" data-min_quantity="<?php echo $extra->getMinQuantity() ?>">
                    <div class="bookly-extras-thumb bookly-js-extras-thumb<?php if ( $extra_count ): ?> bookly-extras-selected<?php endif ?>" tabindex="0">
    ...
    <?php if ( in_array( 'summary', $show ) ) : ?>
        <div class="bookly-box bookly-extras-summary bookly-js-extras-summary"><?php esc_html_e( 'Total Price', 'bookly' ) ?>:<?php if ( $chain_price !== null ): ?> <?php echo Price::format( $chain_price ) ?><?php endif ?><span><?php echo $amount ? ' + ' . Price::format( $amount ) : '' ?></span></div>
    <?php endif ?>
</div>
